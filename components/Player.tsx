...
export const Player: React.FC<PlayerProps> = ({ currentSong, isPlaying, onPlayPause, onNext, onPrev, onToggleLike, onDownload, isLiked }) => {
  ...
  useEffect(() => {
    if (currentSong && audioRef.current) {
        ...
        if (isPlaying) {
            // Promise to prevent play interrupted error
  ...
  return (
    <>
      ...
      {/* Bottom Player */}
      <div className="fixed bottom-0 left-0 right-0 bg-dark/95 backdrop-blur-xl border-t border-white/10 px-4 py-3 z-50 pb-safe">
        <audio 
          ref={audioRef} 
